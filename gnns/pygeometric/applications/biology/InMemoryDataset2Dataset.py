...
class BiologyDataset(Dataset):
    ...
    def process(self) -> None:
        ...
        meta = {"num_data": len(data_list)}
        torch.save(meta, osp.join(self.processed_dir, "meta.pt"))

        for idx, val in enumerate(data_list):
            torch.save(val, osp.join(self.processed_dir, f"data_{idx}.pt"))
    ...
    @property
    def get_meta(self):
        return torch.load(osp.join(self.processed_dir, "meta.pt"))

    def len(self) -> int:
        return self.get_meta["num_data"]
...
if __name__ == "__main__":
    dataset = BiologyDataset(root="./data/biology", name="Alz")
    dataset[0]
